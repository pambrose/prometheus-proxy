...
Lincheck concurrency specs (`*LincheckTest`) carry the Kotest `Lincheck` tag (`common/Lincheck.kt`) and take minutes, so the `Test` task's default `kotest.tags` expression (`!Lincheck`) leaves them out of `./gradlew test` and CI; run them with `make lincheck-tests` (`-PkotestTags=Lincheck`). Tag any new Lincheck spec the same way.
