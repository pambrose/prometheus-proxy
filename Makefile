.PHONY: default help stop clean clean-all stubs build tibuild refresh jars \
        ...
        gh-status tsconfig distro docker-push homebrew-formula release tree depends lint detekt detekt-baseline \
...
# A local clone of pambrose/homebrew-tap, which `make homebrew-formula` writes the agent formula into.
...
# Hashes the jar attached to the GitHub release rather than build/libs, since that is the file brew downloads.
homebrew-formula: _require-version  ## Write the agent's Homebrew formula for the published release into HOMEBREW_TAP_DIR
	...
	curl -fsSL -o "$$jar" "$$url" || { echo "ERROR: could not download $$url; publish the $(VERSION) release first" >&2; exit 1; }; \
	sha="$$(shasum -a 256 "$$jar" | cut -d' ' -f1)"; \
	mkdir -p "$(HOMEBREW_TAP_DIR)/Formula"; \
	sed -e 's/@VERSION@/$(VERSION)/g' -e "s/@SHA256@/$$sha/g" etc/homebrew/prometheus-agent.rb \
		> "$(HOMEBREW_TAP_DIR)/Formula/prometheus-agent.rb"; \
	echo "Wrote $(HOMEBREW_TAP_DIR)/Formula/prometheus-agent.rb for $(VERSION) (sha256 $$sha)"
