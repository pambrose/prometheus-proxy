...
- Switch the proxy/agent Docker images to the prebuilt `eclipse-temurin:17-jre` base (no build-time `apk add openjdk17-jre`, so builds are faster and not subject to Alpine-mirror stalls). The Ubuntu-based Temurin image publishes amd64, arm64, s390x, and ppc64le manifests, so the full multi-arch `make docker-push` platform set resolves and the images run on Apple Silicon (Alpine JRE images cover only amd64/arm64)
- Pin the `eclipse-temurin:17-jre` base image by its manifest-list digest in `etc/docker/{agent,proxy}.df` for reproducible builds (the tag is kept inline for readability), and drop the no-op `-XX:+UnlockExperimentalVMOptions` / `-XX:+UseG1GC` ENTRYPOINT flags (G1 is the JDK 17 default GC and the remaining flags are non-experimental)
