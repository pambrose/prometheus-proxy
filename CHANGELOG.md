...
- **The dashboard can refuse requests for unknown host names, against DNS rebinding.** An attacker who points their own DNS name at the dashboard's address gets a browser to send that name as both Origin and Host, which the WebSocket Origin check allows. The new opt-in `proxy.dashboard.allowedHosts` makes the dashboard answer only to the listed names, IP addresses, `localhost`, and the hosts of `allowedOrigins`; any other Host gets a 403 on every route, with a WARN logged once per host. It is empty, and the check off, by default
